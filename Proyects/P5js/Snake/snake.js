...
class Food{
	...
	moveFoodToRandom(cantSquaresX, cantSquaresY, snake){
		...
		if(snake.tail.length < cantSquaresX * cantSquaresY){
			while(continueTrying){
				positions = this.getRandomPositions(cantSquaresX, cantSquaresY);
				this.x = positions[0];
				this.y = positions[1];
...
class Board{
	constructor(cantSquaresX, cantSquaresY, squareSize, percentage){
	...
	update(){
		...
		if(this.instructions === undefined || this.instructions.length == 0){
			...
			this.instructions = botTurnLazyOptimized(this.instructions, boardValues, this.percentage);
		}
